...
func _ready():
	...
	point_view.add_point(Vector3(0,1,0), Colors.PURPLE_B);
	point_view.add_point(Vector3(0,0,1), Colors.PURPLE_B);
	
	surface_tool.begin(Mesh.PRIMITIVE_TRIANGLES);
	surface_tool.set_color(Colors.BLUE_B);
	add_quad(Vector3i(0,0,0), 0);
	add_quad(Vector3i(0,0,0), 1);
	add_quad(Vector3i(0,0,0), 2);
